CC = gcc
CFLAGS = -Wall -g
TEST = simple_dma_tests
SOURCE = simple_dma_tests.c
TEST2 = user_dma_ops
SOURCE2 = user_dma_ops.c
...
$(TEST2): $(SOURCE2)
	$(CC) $(CFLAGS) -o $(TEST2) $(SOURCE2) -lm

clean:
	rm -f $(TEST) $(TEST2)
	rm -f *.mcs *.prm *.jou *.log
...
.PHONY: all clean load_drivers run_tests run rescan program program_flash program_with_artifact
