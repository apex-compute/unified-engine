#!/usr/bin/env python3
"""
Kokoro-82M CPU reference model.

A self-contained port of hexgrad/kokoro's model code (StyleTTS2 text/prosody
encoders + ISTFTNet decoder), used as the golden reference the accelerator
path in kokoro_fpga.py is validated against, and as the source of the weight
tensors that path uploads (it reads them straight off a loaded KokoroModel's
submodules -- see kokoro_fpga.run_fpga_forward's `model` argument).

Only one thing is still pulled from outside: `transformers` for AlbertModel/
AlbertConfig (the PL-BERT text encoder backbone) -- a general ML library, not
kokoro-specific. The G2P frontend (misaki + espeak-ng) lives in kokoro_test.py
alongside the weight/voice download, since it is a text frontend rather than
model forward-pass code.

Driven by kokoro_test.py; this module has no CLI of its own.
"""
import json
import math
