...
quantized policy \emph{acts} the same. Table~\ref{tab:libero} is the closed-loop
comparison---the FPGA \iffour{} policy vs.\ the CUDA \bfteen{} reference over
10 prompts (9 \texttt{libero\_spatial} + 1 \texttt{libero\_object}), one episode
each, with identical seed and fixed denoise noise so both platforms see
byte-identical observations. We score each by task success and by the
...
vision, 832-token prefix. GFLOP/s is on the hardware-issued (padded) FLOPs.}
\label{tab:timing-fpga}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Model section} & \textbf{Time (s)} & \textbf{GFLOP/s} \\
\midrule
Vision encoder          & 38.0  & 22.4 \\
Prefix (Gemma-2B)       & 147.6 & 23.0 \\
Action expert (denoise) & 22.1  & 21.9 \\
\midrule
\textbf{End-to-end}     & 207.7 & 22.8 \\
\textbf{Throughput (inf/s)} & \multicolumn{2}{c}{0.0048} \\
\bottomrule
\end{tabular}
\end{table}
