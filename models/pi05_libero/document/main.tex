...
throughput --- projected ASIC implementation of the \ue{} v1 IP: 8~engines at
$1.6$\,GHz ($64$~MACs each; $1638.4$\,GFLOP/s aggregate peak), $100$\,GB/s
DRAM. Batch~1, 10 denoise steps. Linear $61.5\times$ compute scaling of the
measured FPGA run at its achieved \%-of-peak; no section is bandwidth-bound
(see below).}
...
\textbf{Model section} & \textbf{Time (ms)} & \textbf{GFLOP/s} \\
\midrule
Vision encoder          & 575 & 1\,479 \\
Prefix (Gemma-2B)       & 2\,236 & 1\,520 \\
Action expert (denoise) & 335 & 1\,444 \\
\midrule
\textbf{End-to-end}     & 3\,146 & 1\,505 \\
\textbf{Throughput (inf/s)} & \multicolumn{2}{c}{0.32} \\
\bottomrule
\end{tabular}
\end{table}
...
\iffour{} weights ($\sim$0.53\,B/param): the prefix reads $\sim$1.1\,GB once
...
demand $<$10\,GB/s of the $100$ available), so no stage becomes
bandwidth-bound and times extrapolate linearly at the FPGA's measured
$88$--$93\%$ of peak, assuming ideal 8-way engine scaling: $3.15$\,s per
inference ($0.32$\,inf/s), $61.5\times$ the FPGA, within $24\times$ of the
RTX~5070's $7.68$\,inf/s.
...
a sub-4\,GB budget. Tables~\ref{tab:budget}--\ref{tab:timing-silicon} complete
