...
The [encoder precision study](validation/20260915_encoder_accuracy/README.md) adds optional FIR kernels. Sorted accumulation reduces eight-file pooled error to **22.64%** at RTF **4.825**, with two noisy-case regressions. Original filter coefficients split into BF16 high/residual values reduce the separate short clip from **20.01% to 16.10%**, at RTF **6.46**; broader matrix-filter validation is pending. The existing deployment defaults are preserved.
