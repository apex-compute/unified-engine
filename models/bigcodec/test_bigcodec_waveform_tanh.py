"""Mono output compaction, exact BF16 math, and AXI-beat padding isolation."""
...
from bigcodec_compile import (Graph, Operation, Tensor, WAVEFORM_BEAT_LANES,
...
class MemoryEngine(LSTMMemoryEngine):
    """Independent byte-DMA interpreter, including32-byte AXI256 fragments."""
    def __init__(self):
        super().__init__()
        self.transfers = []
    ...
    def _dma(self, source, destination, count, *, read, memcpy_length_bytes,
        ...
        for index in range(size // chunk):
            if read:
                ...
                value = self.sram[(source + index * chunk) // 2:][:chunk // 2]
...
class WaveformTanhTests(unittest.TestCase):
    ...
    def test_compacted_output_matches_bf16_math_and_zeros_all_padding(self):
        for rows in (4, 200, 256, 260, 516):
            with self.subTest(rows=rows):
                ...
                reads = [item for item in engine.transfers if item[0] and item[4:] == (32, 128)]
                writes = [item for item in engine.transfers if not item[0] and item[4:] == (32, 128)]
                self.assertEqual(sum(item[3] for item in reads), rows * 32)
                self.assertEqual(sum(item[3] for item in writes), rows * 32)
                self.assertEqual(engine.native_calls.count(udc.LALU_MODE.MODE_RECIP), rows // 4)
    ...
    def test_compiler_reserves_no_dram_scratch_and_prepares_padding_mask(self):
        ...
        mask = engine.view(operation.plan['mask_address'], TANH_CHUNK_ELEMENTS).reshape(-1, 16)
    ...
    def test_capture_has_one_halt_and_fourfold_fewer_reciprocals(self):
        ...
        self.assertEqual(reciprocals[0], 4 * reciprocals[1])
        self.assertGreater(reciprocals[1], 0)
        self.assertLess(instructions[1], instructions[0] // 3)

    def test_invalid_shapes_and_aliases_fail_before_dma(self):
        engine, _, args = fixture(200)
        for index, replacement in ((2, 0), (2, 201), (2, True), (2, 1.5),
