...
def main():
    ...
    # E4B ships LM-only by DEFAULT; the vision + audio encoders are opt-in
    # (the unified bin shrank to ~51 MB after the §7/§8 instruction
    # minimization, so it now fits). Mirror gemma4_e4b_test.py main(): gate
    # the encoder modes behind GEMMA4_E4B_ALLOW_ENCODER=1 so a plain LM run
    # never pays the encoder load. The bin must already contain the encoder
    # sections (built by gemma4_e4b_test.py with GEMMA4_E4B_ALLOW_ENCODER=1).
    if (vision_on or audio_on) and os.environ.get("GEMMA4_E4B_ALLOW_ENCODER", "0") != "1":
        raise SystemExit(
            "E4B runs LM-only by default. Set GEMMA4_E4B_ALLOW_ENCODER=1 to enable "
            "the vision/audio encoders (the bin must have been built with the "
            "encoders, i.e. gemma4_e4b_test.py GEMMA4_E4B_ALLOW_ENCODER=1). "
            "Otherwise drop the --vision-enable / --audio-enable flag.")
    ...
    if not (os.path.exists(instr_bin) and os.path.exists(instr_meta)):
        raise SystemExit(
            ...
            f"  # LM-only (default; fits within 4 GB DRAM):\n"
            f"  python models/gemma4_e4b/gemma4_e4b_test.py\n"
            f"  # Or with vision+audio (requires GEMMA4_LM_ONLY_BIN=0 and a\n"
            f"  # smaller prefill_max_seq_len in gemma4_e4b_config.json):\n"
            f"  GEMMA4_LM_ONLY_BIN=0 python models/gemma4_e4b/gemma4_e4b_test.py\n"
