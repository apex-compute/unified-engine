        ...
        for trial in range(args.trials):
            ...
            if not args.no_video and replay and not args.dry_run:
                try:
                    ...
                    vpath = (vdir / f"verapulse_{args.task_suite}"
                                    f"_t{task_id}_e{trial}_{suffix}.mp4")
                    imageio.mimwrite(vpath, replay, fps=10)
                    # PRINT THE PATH. The name is only knowable AFTER the episode ends --
                    # `suffix` is the outcome -- so reconstructing it by hand means
                    # scrolling back for the task id and trial and then guessing whether
                    # it succeeded. Absolute, so it can be pasted straight into a player.
                    print(f"[eval]   video -> {vpath.resolve()}  "
                          f"({len(replay)} frames, {len(replay) / 10:.1f}s @10fps)",
                          flush=True)
