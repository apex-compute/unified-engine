...
class VeraPulse_UnifiedEngine(UnifiedEngine):
    ...
    # Minimal local stand-ins for the shared multi-engine scaffolding so the PREFIX
    # column-sharding below can be written and statically checked against the real API.
    # Everything here is expected to be REPLACED by the vision agent's version; only the
    # PREFIX-specific pieces (_prefix_column_blocks, the PREFIX entry in
    # STAGE_MAX_ENGINES) are meant to survive the merge.
    # ==================================================================================
    NUM_ENGINES = 1
    ...
    # Worker DRAM pool base. NOT the library default (DRAM_START_ADDR + 0x10000000 =
    # 0x90000000): this model's params blob runs 0x80000000..~0xC4000000 and its tensor
    # arena 0xC8000000..0xD6000000, so the DEFAULT WORKER ARENAS WOULD LAND INSIDE THE
    # WEIGHTS. Park the pool above the tensor arena, inside the (2.85 GB, hugely
    # over-provisioned) program region, leaving the primary 0xD6000000..0x100000000
    # (680 MB) for its own programs.   <-- vision agent: this needs a real decision.
    WORKER_DRAM_BASE   = 0x100000000
    WORKER_DRAM_STRIDE = 0x08000000
    ...
    def _num_engines(self, stage):
        stage = stage.upper()
        n = getattr(self, f"{stage}_NUM_ENGINES", None)
        n = int(self.NUM_ENGINES if n is None else n)
        n = max(1, min(n, self.STAGE_MAX_ENGINES.get(stage, 1)))
        if stage == "PREFIX":
            n = min(n, self._prefix_column_blocks())
        return n

    def _worker_engine_pool(self, n):
        """ONE shared worker pool for the whole run, cached and grown in place.

        Building a SECOND pool restarts each worker's DRAM allocator at offset 0 and
        overwrites the earlier stage's worker programs byte for byte, so stage 1's next
        execution launches stage 2's code and every engine deadlocks on a FLAG_CHECK
        that has no timeout. See MultiEngineScheduler.__init__'s note."""
        pool = list(getattr(self, "_vp_worker_pool", []) or [])
        for i in range(len(pool) + 1, n):
            base = self.WORKER_DRAM_BASE + (i - 1) * self.WORKER_DRAM_STRIDE
            pool.append(UnifiedEngine(
                BASE_ADDR=user_dma_core.UE_0_BASE_ADDR + i * 0x00010000,
                params_dram_base=base,
                tensor_dram_base=base + 0x02000000,
                program_dram_base=base + 0x04000000,
            ))
        self._vp_worker_pool = pool
        return list(pool[: n - 1])

    def _make_stage_scheduler(self, stage, ne):
        """One scheduler per stage, cached, all sharing the single worker pool."""
        cache = self.__dict__.setdefault("_vp_scheds", {})
        if stage in cache:
            sched = cache[stage]
            assert sched.num_engines == ne, (
                f"stage {stage} already has a scheduler at {sched.num_engines} engines")
            return sched
        guard = None
        if ne > 1:
            # Every UnifiedEngine ctor DMA-writes 16 KB of noise to a HARDCODED
            # 0x80000000 -- this model's first stored weight. Snapshot/restore around
            # construction, in a BFLOAT16 buffer (dma_read numerically CASTS any other
            # dtype, so "restoring" a uint8 snapshot corrupts more than it fixes).
            guard = torch.zeros(0x10000 // 2, dtype=torch.bfloat16)
            self.dma_read(self.c2h_device, user_dma_core.DRAM_START_ADDR, guard, 0x10000)
        workers = self._worker_engine_pool(ne) if ne > 1 else []
        if guard is not None:
            self.dma_write(self.h2c_device, user_dma_core.DRAM_START_ADDR, guard, 0x10000)
        sched = mes.MultiEngineScheduler(
            self, num_engines=ne, workers=workers or None,
            allow_more_than_two_engines=True,
            worker_dram_base=self.WORKER_DRAM_BASE,
            worker_dram_stride=self.WORKER_DRAM_STRIDE,
        )
        cache[stage] = sched
        return sched

    def _vp_barrier(self, ue, engine_idx, ne):
        """Emits NOTHING at ne == 1 (byte-identity). Above that the scheduler owns
        every rendezvous, so a hand-rolled barrier is always a mistake."""
        if ne == 1:
            return
        raise AssertionError(
            "_vp_barrier: barriers above ne=1 come from the scheduler's region API")
    # ============================ END STUB ============================================
...
def configure_engines(ue, spec):
    """Apply an engine spec to a model instance.

    STUB -- owned by the vision agent, delete on merge; the PREFIX entries are the part
    meant to survive. ``spec`` is the parsed args namespace (or anything carrying the
    same attribute names).
    """
    ue.NUM_ENGINES = max(1, int(getattr(spec, "engines", 1) or 1))
    if getattr(spec, "pref_8", False):
        assert getattr(spec, "prefix_engines", None) in (None, 8), \
            "--pref_8 and --prefix-engines disagree"
        ue.PREFIX_NUM_ENGINES = 8
    elif getattr(spec, "prefix_engines", None):
        ue.PREFIX_NUM_ENGINES = int(spec.prefix_engines)
    # Multi-engine programs live in N separate capture buffers; a bin holds only
    # engine 0's, so loading it back would run a primary that barriers against workers
    # that were never launched -- a deadlock with no timeout, not a wrong answer.
    if getattr(spec, "dump_bins", False) or getattr(spec, "from_bin", False):
        assert ue._num_engines("PREFIX") == 1 and ue.NUM_ENGINES == 1, (
            "--dump-bins/--from-bin are single-engine only: a program bin captures "
            "engine 0 alone and drops every worker program")
    n = ue._num_engines("PREFIX")
    if n > 1:
        _original_print(f"  [engines] PREFIX sharded over {n} engine(s) "
                        f"(column axis; {ue._prefix_column_blocks()} column blocks "
                        f"available)")
...
def main():
    ...
    ap.add_argument("--engines", type=int, default=1,
                    help="engines for EVERY stage; per-stage flags below override it")
    ap.add_argument("--pref_8", action="store_true",
                    help="STAGE ISOLATION for the prefix: run the prefix's projection "
                         "matmuls on 8 engines (column-sharded) and leave vision and "
                         "denoise single-engine. Shorthand for --prefix-engines 8.")
