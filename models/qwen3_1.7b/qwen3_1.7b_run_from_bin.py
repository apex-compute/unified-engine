...
pre-compiled instruction bin from disk, and run prefill + decode. Deterministic
on-FPGA decode only: the repetition penalty is folded into the LM-head matmul bias
(HW argmax of logits+bias, no host logit readback). No host sampling.

Requirements on disk (relative to this file):
  - ``qwen3_1.7b_bin/qwen3_1.7b_config.json``   model config
  - ``qwen3_1.7b_bin/params.bin`` quantized weight bin
  - ``qwen3_1.7b_bin/programs.bin`` pre-compiled program bin
  - ``qwen3_1.7b_bin/programs.json`` compile-meta sidecar
...
Usage:
  ...
  python qwen3_1.7b_run_from_bin.py --pure-greedy        # disable the penalty
  python qwen3_1.7b_run_from_bin.py --pen-alpha 1.0 --greedy-until 512 --rep-window 256
"""
...
from user_dma_core import DMA_DEVICE_H2C, TYPE, UE_FMAX_CONTEXT_SIZE, UE_MODE, UE_VECTOR_SIZE, UE_ARGMAX_INDEX, SCALE_BRAM_ELEMENTS, configure_device, ue_35bit_addr_shifter
...
def _set_dram_layout_for_device(device: str) -> None:
    ...
    if device == "efinix":
        QWEN3_PARAMS_BASE = 0x00000000
        QWEN3_TENSOR_BASE = 0x45000000
        QWEN3_PROGRAM_BASE = 0x70000000
    else:
        QWEN3_PARAMS_BASE = 0x00000000
        QWEN3_TENSOR_BASE = 0x58000000
        QWEN3_PROGRAM_BASE = 0xE0000000
...
class Qwen3_1_7b_UnifiedEngine(UnifiedEngine):
    ...
    # the HW argmax register holds the penalized token id, no logit readback. The bin is the
    # released artifact; this runner only maintains the bias buffer. See the design note. ----
    ...
    def tensor_init(self) -> None:
        ...
        # exactly as in qwen3_1.7b_test.py, so its address matches the one baked into the bin.
...
def main():
    ...
    _original_print(f"Using DMA device:")
    ...
    _original_print(f"  DRAM layout: params=0x{QWEN3_PARAMS_BASE:08X}, tensor=0x{QWEN3_TENSOR_BASE:08X}, "
