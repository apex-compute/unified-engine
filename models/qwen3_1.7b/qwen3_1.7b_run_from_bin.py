...
``qwen3_1.7b_test.py`` once on a build machine with HF access to produce
them; this script then runs entirely offline.

Bin layout (matches ``compile_instructions`` in qwen3_1.7b_test.py):
...
def check_bins_early(weights_bin_full):
    ...
    ``qwen3_1.7b_test.py`` on a build machine with HF access.
    """
    missing = []
...
class Qwen3_1_7b_UnifiedEngine(UnifiedEngine):
    ...
    def __init__(self, script_dir=None, weights_bin=None):
        ...
        # Fixed dynamic-PBI GPRs (mirror qwen3_1.7b_test.py / compare_qwen3_1.7b.py).
...
def main():
    ...
    inst_meta_path = os.path.join(SCRIPT_DIR, paths_cfg.get("instruction_meta",
                                  "qwen3_1.7b_bin/qwen3_1.7b_instruction.json"))
