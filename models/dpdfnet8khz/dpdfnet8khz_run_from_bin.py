#!/usr/bin/env python3
"""Enhance audio files or spectrum frames with one resident DPDFNet2 8 kHz bin.
...
def _load_frames(path: Path) -> np.ndarray:
    ...
    if value.shape == (1, 1, 81, 2):
        value = value[None]
    if value.ndim != 5 or tuple(value.shape[1:]) != (1, 1, 81, 2):
        raise ValueError(
            "spectrum input must have shape [1,1,81,2] or "
            "[frames,1,1,81,2]")
    if value.shape[0] == 0:
...
def main(argv=None, *, default_bin=None, expected_precision=None) -> None:
    ...
    parser.add_argument(
        "--bin", type=Path,
        default=default_bin or HERE / "dpdfnet8khz_bin" / "dpdfnet2_8khz-andromeda.bin")
    parser.add_argument(
        "--input", type=Path, required=True,
        help="audio file (WAV/FLAC), or NumPy spectra [T,1,1,81,2]")
    parser.add_argument(
        "--output", type=Path,
    ...
    if args.output is None:
        stem = (f"enhanced_8khz_{expected_precision}" if expected_precision
                else "enhanced")
        args.output = HERE / "dpdfnet8khz_bin" / (stem + (".wav" if audio_mode else "_spec.npy"))
    ...
    audio_input = None
    if audio_mode:
        from dpdfnet8khz_audio import read_audio, synthesize_audio
    ...
    validate_runtime_hardware(payload, hw_info.axi_data_width_bits)
    print(
        f"DPDFNet2 8 kHz FPGA neural inference, frames={frames.shape[0]}, "
