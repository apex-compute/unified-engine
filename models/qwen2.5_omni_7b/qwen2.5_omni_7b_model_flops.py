...
(``_ckpt(f"L{li}:qkv_proj")`` etc. in qwen2.5_vl_3b_vision.py and
qwen2.5_vl_3b_lm.py, read from source), so a dict here lines up directly with
...
def vision_flops_by_phase(cfg: dict, patches: int | None = None,
    ...
    checkpoint names (qwen2.5_vl_3b_vision.py's ``_ckpt(...)`` calls):
...
def _lm_layer_breakdown(cfg: dict, tokens: int,
    ...
    names _make_ckpt emits in qwen2.5_vl_3b_lm.py -- shared by prefill and
...
def prefill_flops_by_phase(cfg: dict, seq_len: int) -> dict[str, int]:
    ...
    decode's -- confirmed from source (qwen2.5_vl_3b_lm.py's _emit_layer,
