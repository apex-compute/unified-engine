...
FORMAT = "andromeda.dpdfnet2.streaming-v2"
...
def padded_row_patterns(source: TensorLayout, destination: TensorLayout,
                        source_indices, destination_indices):
    """Recognize copies of complete logical rows, preserving column order.

    The caller must separately prove that copying source padding is safe.
    Partial rows and permutations within a row use the element-copy path.
    """
    width = source.logical_last
    if (width != destination.logical_last
    ...
    if source_indices.size != destination_indices.size:
        raise ValueError("row-copy source/destination maps differ in length")
    if not source_indices.size or source_indices.size % width:
        return None
    columns = np.arange(width, dtype=np.int64)
    row_indices = []
    for layout, indices in ((source, source_indices),
                            (destination, destination_indices)):
        rows = indices.reshape(-1, width)
        starts = rows[:, 0]
        if (np.any(starts < 0)
                or np.any(starts + layout.padded_last > layout.physical_elements)
                or np.any(starts % layout.padded_last)
                or not np.array_equal(rows, starts[:, None] + columns)):
            return None
        row_indices.append(starts // layout.padded_last)
    return copy_patterns(*row_indices)
...
class DeviceEmitter:
    ...
    def __init__(self, layouts: dict[str, TensorLayout], tensor_end: int,
        ...
        # Only completed graph values are recorded, never reusable scratch.
        # Runtime input packing and the initial model image zero these lanes.
        self._zero_padding = {name for name in ("spec", "state_in") if name in layouts}

    def mark_padding_zero(self, layout: TensorLayout) -> None:
        if not layout.name.startswith("@") and self.layouts.get(layout.name) == layout:
            self._zero_padding.add(layout.name)
    ...
    def emit_scatter(self, source: TensorLayout, destination: TensorLayout,
        ...
        if (source.logical_last == source.padded_last
                or source.name in self._zero_padding):
            rows = padded_row_patterns(
                source, destination, source_physical, destination_physical)
            if rows is not None:
                row_bytes = source.padded_last * 2
                max_rows = udc.URAM_NEAR_FULL_SIZE // row_bytes
                if max_rows and all(
                        stride == 1 or (
                            row_bytes <= udc.UE_STRIDE_CHUNK_MAX_BYTES
                            and stride * row_bytes <= udc.UE_STRIDE_JUMP_MAX_BYTES)
                        for _, _, _, stride in rows):
                    for source_row, destination_row, count, stride in rows:
                        for offset in range(0, count, max_rows):
                            take = min(max_rows, count - offset)
                            self.engine.accelerator_memory_to_sram(
                                source.address + (source_row + offset * stride) * row_bytes,
                                0, 0, memcpy_length_bytes=take * row_bytes,
                                stride_bytes_per_chunk=row_bytes if stride != 1 else 0,
                                stride_jump_bytes=stride * row_bytes if stride != 1 else 0)
                            self.engine.sram_to_accelerator_memory(
                                0, destination.address + (destination_row + offset) * row_bytes,
                                0, memcpy_length_bytes=take * row_bytes)
                    return
...
def validate_hardware(payload: dict) -> None:
    ...
    if payload.get("format") != FORMAT or payload.get("model") != "dpdfnet2":
        raise RuntimeError(
            "incompatible DPDFNet2 streaming artifact; rebuild with "
            "dpdfnet_compile.py --force (v2 fixes silent-energy square roots "
            "and packed-state alignment)")
    ...
    def execute(self, spectrum: torch.Tensor | np.ndarray) -> torch.Tensor:
        if self._failure_reason is not None:
            raise FloatingPointError(
                f"DPDFNet backend is stopped: {self._failure_reason}. "
                "Create a new backend to reload the model image and recurrent state")
        ...
        self.output_reads += 1
        result = unpack_tensor(flat, output)
        # Padding is outside the logical tensor and can contain nonfinite
        # intermediate values. Only the model's actual output is checked.
        if not torch.isfinite(result).all():
            self._failure_reason = (
                f"nonfinite output at frame {self.program_kicks} (1-based): "
                f"NaN={int(torch.isnan(result).sum())}, "
                f"+Inf={int(torch.isposinf(result).sum())}, "
                f"-Inf={int(torch.isneginf(result).sum())}")
            # The resident program has already committed its recurrent state;
            # another frame must not consume that potentially poisoned state.
            raise FloatingPointError(
                f"DPDFNet {self._failure_reason}. "
                "Create a new backend to reload the model image and recurrent state")
        return result
