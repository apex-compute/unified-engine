...
class GraphCompiler:
    ...
    def __init__(self, onnx, model, digest: str, *, conv_precision="auto"):
        ...
        self.state_copy_scratch = self._allocate_aux("@state_copy/scratch", (64,))
        ...
        self.state_copy_resources = self._prepare_state_copies()
    ...
    def _plan_auxiliary_layouts(self):
        ...
        for index, node in enumerate(self.model.graph.node):
            ...
            if node.op_type == "GRU":
                ...
                sequence = self.shapes[node.input[0]][0]
                shapes = ((sequence, 192), (1, 192)) + ((1, hidden),) * 5
                self.gru_aux[index] = {
    ...
    def prepare_state_copy(self, source, destination, source_start,
                           destination_start, count):
        """Plan a packed-state copy using beat-aligned DMA and lane selectors.

        Canonical ONNX state segments need not begin on an AXI beat. The
        descriptor cannot encode their low address bits, so their boundary
        rows are assembled by the compute engine and written as full rows.
        All constants are allocated before program capture begins.
        """
        if len(source.shape) != 1 or len(destination.shape) != 1:
            raise ValueError("packed-state copies require one-dimensional tensors")
        if (min(source_start, destination_start, count) < 0
                or source_start + count > source.logical_elements
                or destination_start + count > destination.logical_elements):
            raise ValueError("packed-state copy exceeds its logical tensor")
        resource = {
            "source": source, "destination": destination,
            "source_start": source_start, "destination_start": destination_start,
            "prefix": 0, "rows": [],
        }
        # A 64-byte boundary is safe on both supported AXI widths. Keep the
        # bulk DMA path for the many state segments that already align.
        if source_start % 32 == destination_start % 32 == 0:
            prefix = count // 32 * 32
            ...
            count -= prefix
        while count:
            destination_row, destination_column = divmod(destination_start, 64)
            take = min(count, 64 - destination_column)
            row = {"destination_row": destination_row, "keep": None, "pieces": []}
            if destination_column or take != 64:
                keep = torch.eye(64, dtype=torch.bfloat16)
                keep[destination_column:destination_column + take] = 0
                row["keep"] = self.emitter.allocate_constant(keep)
            consumed = 0
            while consumed < take:
                source_row, source_column = divmod(source_start + consumed, 64)
                piece_count = min(take - consumed, 64 - source_column)
                if (source_row + 1) * 64 > source.physical_elements:
                    raise ValueError("packed-state source row exceeds tensor padding")
                selector = torch.zeros(64, 64, dtype=torch.bfloat16)
                offsets = torch.arange(piece_count)
                selector[destination_column + consumed + offsets,
                         source_column + offsets] = 1
                row["pieces"].append((
                    source_row, self.emitter.allocate_constant(selector)))
                consumed += piece_count
            if (destination_row + 1) * 64 > destination.physical_elements:
                raise ValueError("packed-state destination row exceeds tensor padding")
    ...
    def _prepare_state_copies(self):
        ...
        for node in self.model.graph.node:
            if node.op_type == "Slice" and node.input[0] == "state_in":
                starts = self._integer_initializer(node.input[1])
                axes = self._integer_initializer(node.input[3])
                steps = self._integer_initializer(node.input[4])
                if starts.size != 1 or axes.tolist() != [0] or steps.tolist() != [1]:
                    raise RuntimeError("DPDFNet packed-state Slice contract changed")
                destination = self.layout(node.output[0])
                resources[node.output[0]] = [self.prepare_state_copy(
                    self.layout("state_in"), destination,
                    int(starts[0]), 0, destination.logical_elements)]
            elif node.op_type == "Concat" and node.output[0] == "state_out":
                destination = self.layout("state_out")
                copies = []
                offset = 0
                for name in node.input:
                    source = self.layout(name)
                    copies.append(self.prepare_state_copy(
                        source, destination, 0, offset, source.logical_elements))
                    offset += source.logical_elements
                if offset != destination.logical_elements:
                    raise RuntimeError("DPDFNet packed-state Concat coverage changed")
                resources["state_out"] = copies
        return resources

    def emit_state_copy(self, resource):
        engine = self.emitter.engine
        source, destination = resource["source"], resource["destination"]
        if resource["prefix"]:
            engine.accelerator_memcpy(
                source.address + resource["source_start"] * 2,
                destination.address + resource["destination_start"] * 2,
                resource["prefix"] * 2)
        for row in resource["rows"]:
            target = destination.address + row["destination_row"] * 128
            accumulated = row["keep"] is not None
            if accumulated:
                engine.matmat_mul_core(
                    M=1, K=64, N=64, A_DRAM_ADDR=target,
                    B_DRAM_ADDR=row["keep"], OUTPUT_DRAM_ADDR=target,
                    is_B_quantized=False)
            for source_row, selector in row["pieces"]:
                piece_output = self.state_copy_scratch.address if accumulated else target
                engine.matmat_mul_core(
                    M=1, K=64, N=64,
                    A_DRAM_ADDR=source.address + source_row * 128,
                    B_DRAM_ADDR=selector, OUTPUT_DRAM_ADDR=piece_output,
                    is_B_quantized=False)
                if accumulated:
                    engine.eltwise_core_dram(
                        M=1, N=64, dram_a=target, dram_b=piece_output,
    ...
    def _emit_transpose64_columns(self, source_address, output_address, columns,
                                  rows=64):
        """Materialize the needed prefix of a 64x64 BF16 transpose.

        Complex packing consumes only two transposed rows, and unpacking
        consumes at most 32. Keep the shared transpose's exact matvecs and
        SRAM placement while omitting output rows with no consumers.
        """
        if not 1 <= columns <= 64 or not 1 <= rows <= 64:
            raise ValueError("partial transpose requires 1..64 rows and columns")
        engine = self.emitter.engine
        engine.accelerator_memory_to_sram(self.identity_address, 0, 64 * 64)
        engine.accelerator_memory_to_sram(source_address, 0x80000, rows * 64)
        if rows != 64:
            engine.accelerator_memory_to_sram(
                self.emitter.zero_address, 0x2000, columns * 64)
        for column in range(columns):
            engine.start_queue_for_bf16_matvec_operation(
                ...
                output_sram_wb_addr=0x2000 + column * 128,
                K=64, N=rows, stride_z=64)
        engine.sram_to_accelerator_memory(
            0x2000, output_address, columns * 64)
    ...
    def emit_view(self, node_index, node):
        ...
        if node_index in self.pair_pack_aux:
            ...
            for chunk, row_start in enumerate(range(0, source.rows, 32)):
                ...
                self.emitter.engine.sram_to_accelerator_memory(
                    ...
                    memcpy_length_bytes=take * 128)
                self._emit_transpose64_columns(
                    resource["input"].address,
                    resource["transposed"].address, 2, rows=take)
                destination = output.address + chunk * 128
                self.emitter.engine.matmat_mul_core(
                    M=1, K=64, N=64,
        ...
        if node_index in self.pair_unpack_aux:
            ...
            for chunk, row_start in enumerate(range(0, output.rows, 32)):
                ...
                self._emit_transpose64_columns(
                    resource["input"].address,
                    resource["transposed"].address, take, rows=2)
                self.emitter.engine.accelerator_memory_to_sram(
                ...
                self.emitter.engine.sram_to_accelerator_memory(
                    ...
                    memcpy_length_bytes=take * 128)
            # The input tile is zero apart from its first two rows. Its
            # transpose therefore has exactly zero padding after each pair.
    ...
    def emit_slice(self, node):
        source, output = self.layout(node.input[0]), self.layout(node.output[0])
        if node.input[0] == "state_in":
            self.emitter.emit_zero(output)
            for resource in self.state_copy_resources[node.output[0]]:
                self.emit_state_copy(resource)
            return
        starts = self._integer_initializer(node.input[1])
    ...
    def emit_concat(self, node_index, node):
        ...
        output = self.layout(node.output[0])
        axis = int(attrs.get("axis", 0)) % len(output.shape)
        if node.output[0] == "state_out":
            self.emitter.emit_zero(output)
            for resource in self.state_copy_resources["state_out"]:
                self.emit_state_copy(resource)
            self.emitter.mark_padding_zero(output)
            return
    ...
    def emit_sqrt(self, index, node):
        ...
        # DPDFNet takes square roots of nonnegative spectrum energies. Keep
        # the original energy for the final multiply: sqrt(0) must be zero,
        # not the NaN produced by 0 * rsqrt(0). Only bound the reciprocal's
        # denominator. The smallest normal BF16 immediate is safe for LALU
        # CLAMP (subnormal immediates are not normalized by that datapath).
        self.emitter.engine.activation_core(
            M=source.physical_elements // udc.UE_VECTOR_SIZE,
            N=udc.UE_VECTOR_SIZE,
            A_DRAM_ADDR=source.address, OUTPUT_DRAM_ADDR=inverse.address,
            IDENTITY_DRAM_ADDR=self.identity_address,
            activation="clamp", clamp_min=float(torch.finfo(torch.bfloat16).tiny))
        self.emitter.emit_identity_lalu(
            inverse, inverse, self.identity_address,
    ...
    def _emit_gru_sigmoid_sram(self, source_sram, output_sram, identity_sram):
        self.emitter.engine.start_queue_for_bf16_matvec_operation(
            max_clear_en=1, fmax_context_addr=0,
            vector_sram_start_addr=source_sram,
            matrix_sram_start_addr=identity_sram,
            output_sram_wb_addr=output_sram, K=64, N=64,
            lalu_mode=udc.LALU_MODE.ACT_NO_X,
            lalu_a=udc.LALU_ACT_SIGMOID_A,
            lalu_b=udc.LALU_ACT_SIGMOID_B)

    def _emit_gru_recurrence_sram(self, *, sequence, direction, input_gates,
                                 recurrent_weights, recurrent_bias,
                                 initial_hidden, output, output_hidden):
        """Keep one direction's recurrent weights and BF16 gates in SRAM.

        These regions are private to this lowering. The loop uses only direct
        SRAM operations and the two explicit per-step DMA transfers, so no
        shared planner can overwrite cached matrices or the recurrent state.
        Every arithmetic result still rounds through BF16 SRAM writeback.
        """
        regions = {
            "hg": (0x11000, 192 * 2),
            "temporary": (0x12000, 64 * 2),
            "candidate_a": (0x13000, 64 * 2),
            "hidden": (0x20000, 64 * 2),
            "xg": (0x88000, 192 * 2),
            "z": (0x89000, 64 * 2),
            "r": (0x89100, 64 * 2),
            "candidate_b": (0x89200, 64 * 2),
            "identity": (0x90000, 64 * 64 * 2),
            "recurrent": (0xA0000, 192 * 64 * 2),
        }
        previous_end = 0
        for address, size in sorted(regions.values()):
            if (address % 128 or address < previous_end
                    or address // 0x80000 != (address + size - 1) // 0x80000
                    or address + size > 0x100000):
                raise RuntimeError("GRU SRAM regions overlap or exceed a bank")
            previous_end = address + size
        hg, temporary, candidate_a, hidden, xg, z, r, candidate_b, identity, recurrent = (
            regions[name][0] for name in (
                "hg", "temporary", "candidate_a", "hidden", "xg", "z", "r",
                "candidate_b", "identity", "recurrent"))
        engine = self.emitter.engine
        engine.accelerator_memory_to_sram(recurrent_weights, recurrent, 192 * 64)
        engine.accelerator_memory_to_sram(self.identity_address, identity, 64 * 64)
        engine.accelerator_memory_to_bias_sram(recurrent_bias, 192)
        engine.accelerator_memory_to_sram(initial_hidden, hidden, 64)
        order = range(sequence) if direction == 0 else range(sequence - 1, -1, -1)
        for timestep in order:
            engine.accelerator_memory_to_sram(input_gates + timestep * 384, xg, 192)
            engine.start_queue_for_bf16_matvec_operation(
                max_clear_en=1, fmax_context_addr=0,
                vector_sram_start_addr=hidden,
                matrix_sram_start_addr=recurrent,
                output_sram_wb_addr=hg, K=64, N=192, bias_enable=True)
            # z/r gates retain separate add and sigmoid BF16 writebacks.
            engine.eltwise_add_core(hg, xg, temporary, 64)
            self._emit_gru_sigmoid_sram(temporary, z, identity)
            engine.eltwise_add_core(hg + 128, xg + 128, temporary, 64)
            self._emit_gru_sigmoid_sram(temporary, r, identity)
            engine.eltwise_mul_core(hg + 256, r, temporary, 64)
            engine.eltwise_add_core(temporary, xg + 256, candidate_a, 64)
            # Match tanh_core_dram: BF16(2*sigmoid(BF16(2*x))-1),
            # including the separate multiply and add writeback boundaries.
            engine.broadcast_mul(2.0, candidate_a, candidate_a, 64)
            self._emit_gru_sigmoid_sram(candidate_a, candidate_a, identity)
            engine.broadcast_mul(2.0, candidate_a, candidate_a, 64)
            engine.broadcast_add(-1.0, candidate_a, candidate_b, 64)
            engine.eltwise_sub_core(hidden, candidate_b, temporary, 64)
            engine.eltwise_mul_core(temporary, z, temporary, 64)
            engine.eltwise_add_core(temporary, candidate_b, hidden, 64)
            engine.sram_to_accelerator_memory(
                hidden, output + (timestep * 2 + direction) * 128, 64)
        engine.sram_to_accelerator_memory(hidden, output_hidden, 64)
    ...
    def emit_gru(self, index, node):
        ...
        xg = self.gru_aux[index]["xg"]
        gate_bytes = 64 * 2
        matrix_bytes = 192 * 64 * 2
        bias_row_bytes = 384 * 2

        for direction in range(2):
            # XW+b is independent of the recurrent state. Load each direction's
            # weights once per <=16-row tile instead of once per timestep.
            # Every row still uses the same BF16 matvec and bias epilogue; the
            # gate values retain the original BF16 writeback boundary.
            for timestep in range(0, sequence, 16):
                self.emitter.engine.matmat_mul_core(
                    M=min(16, sequence - timestep), K=64, N=192,
                    A_DRAM_ADDR=source.address + timestep * gate_bytes,
                    B_DRAM_ADDR=weights.address + direction * matrix_bytes,
                    OUTPUT_DRAM_ADDR=xg.address + timestep * 3 * gate_bytes,
                    C_DRAM_ADDR=biases.address + direction * bias_row_bytes,
                    bias_mode="broadcast_N")
            self._emit_gru_recurrence_sram(
                sequence=sequence, direction=direction,
                input_gates=xg.address,
                recurrent_weights=recurrent.address + direction * matrix_bytes,
                recurrent_bias=biases.address + direction * bias_row_bytes + 192 * 2,
                initial_hidden=initial_h.address + direction * gate_bytes,
                output=output.address,
                output_hidden=output_h.address + direction * gate_bytes)
