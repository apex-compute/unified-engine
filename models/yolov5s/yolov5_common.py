...
def configure_hardware_runtime(*, device: str, dev: str,
                               cycle_override_ns: float | None = None
                               ) -> tuple[float, user_dma_core.HardwareInfo, float]:
    """Initialize runtime geometry from HW_INFO and return the effective clock.

    HW_INFO remains authoritative for the AXI width, queue mode, topology, and
    physical clock.  ``cycle_override_ns`` only overrides the period attached
    to this runner's engine for explicit timing experiments.
    """
    user_dma_core.set_dma_device("efinix" if device == "efinix" else dev)
    detected_clock_ns = user_dma_core.configure_clock_from_hardware()
    info = user_dma_core.configured_hardware_info()
    effective_clock_ns = (detected_clock_ns if cycle_override_ns is None
                          else float(cycle_override_ns))
    if not 0.0 < effective_clock_ns < float("inf"):
        raise ValueError(
            f"cycle override must be finite and > 0, got {cycle_override_ns!r}")
    return effective_clock_ns, info, detected_clock_ns
...
def load_yolov5_config(
    ...
    if config_path is None:
        ...
        model_dirname = "yolov5s" if profile.key == "s" else "yolov5n"
        config_filename = ("yolov5s_config.json" if profile.key == "s"
