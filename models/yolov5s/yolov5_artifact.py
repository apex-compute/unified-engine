...
Python values and tensors: an explicit flattened graph, native mixed IF4/IF8
tensors, anchors, metadata, and one fixed-address whole-graph deployment image
for every closed input profile. The hardware runtime
loads it with ``weights_only=True`` and never
imports/downloads the upstream checkpoint, captures programs, or walks graph
nodes between the launch and the final bundled result read.
"""
...
CANONICAL_MODEL_SHA256 = CANONICAL_ARTIFACTS["s"].model_sha256
CANONICAL_PROGRAM_SHA256 = CANONICAL_ARTIFACTS["s"].program_sha256
CANONICAL_MANIFEST_SHA256 = CANONICAL_ARTIFACTS["s"].manifest_sha256
CANONICAL_MODEL_BYTES = CANONICAL_ARTIFACTS["s"].model_bytes
CANONICAL_PROGRAM_BYTES = CANONICAL_ARTIFACTS["s"].program_bytes

# Source compatibility for external metadata readers. These aliases now refer
# to the one combined deployment image and whole-graph manifest.
CANONICAL_PARAMS_SHA256 = CANONICAL_MODEL_SHA256
CANONICAL_DISPATCH_SHA256 = CANONICAL_MANIFEST_SHA256
CANONICAL_PARAMS_BYTES = CANONICAL_MODEL_BYTES
...
def select_single_bin_profile(payload: dict, resolution=None) -> tuple[str, dict]:
    ...
    The returned dictionary shares logical quantized tensors with the outer
    artifact. Operation shapes and the combined model/program deployment image
...
def _profile_digest_descriptor(key: str, payload: dict) -> dict:
    ...
    return {
        ...
        "model_bytes": int(hardware["model_image"].numel()),
        "model_sha256": hardware["model_sha256"],
        "program_bytes": int(hardware["program_size"]),
        "program_sha256": hardware["program_sha256"],
        "manifest_sha256": precompiled_manifest_sha256(hardware),
    }


def _bundle_sha256(payload: dict) -> str:
    """Bind every embedded shape, deployment image, and graph manifest."""
    profiles = []
...
def compile_single_bin(model: torch.nn.Module, output_path: Path, *,
    ...
    return {
        ...
        "model_bytes": payload["hardware"]["model_image"].numel(),
        "program_bytes": payload["hardware"]["program_size"],
        "profile_program_bytes": {
            key: selected["hardware"]["program_size"]
            for key, selected in ((
...
def validate_single_bin(payload: dict) -> None:
    ...
    hardware = payload["hardware"]
    if (artifact_spec.model_bytes
            and (hardware["model_image"].numel() != artifact_spec.model_bytes
                 or hardware["model_sha256"] != artifact_spec.model_sha256)):
        raise RuntimeError("YOLO artifact deployment image is not canonical")
    if (artifact_spec.program_bytes
            and (hardware["program_size"] != artifact_spec.program_bytes
                 or hardware["program_sha256"] != artifact_spec.program_sha256)):
        raise RuntimeError("YOLO artifact whole-graph program is not canonical")
    if (artifact_spec.manifest_sha256
            and precompiled_manifest_sha256(hardware)
            != artifact_spec.manifest_sha256):
        raise RuntimeError("YOLO artifact whole-graph manifest is not canonical")
    ...
    if (artifact_spec.bundle_sha256
            and computed_bundle != artifact_spec.bundle_sha256):
        raise RuntimeError(
...
def load_single_bin(path: Path) -> dict:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"YOLO single bin not found: {path}. Build it with the matching "
            "yolov5s_compile.py or yolov5n_compile.py entrypoint")
...
def execute_single_bin(payload: dict, image_chw: torch.Tensor, backend, *,
    ...
    """Run the CPU graph interpreter or delegate one whole-graph FPGA kick."""
    ...
    if bool(getattr(backend, "executes_whole_graph", False)):
        return backend.execute(image_chw)

    prepared = _prepared_convs(payload)
