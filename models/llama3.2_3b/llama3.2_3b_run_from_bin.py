#!/usr/bin/env python3
"""Llama-3.2-3B inference from pre-compiled bin files (customer release).

This script is the runtime-only counterpart to ``llama3.2_3b_test.py``. It
...
Required artifacts (must already exist in ``llama3.2_3b_bin/`` next to this
script — they are produced by running ``llama3.2_3b_test.py`` once on a build
machine that has internet/HF access):

  * ``weights_llama3.2_3b_hf.bin``    IF4-quantized weights + bf16 embedding (~2.3 GiB)
  * ``llama3.2_3b_instruction.bin``   unified PBI instruction bin (prefill + decoder buckets)
  * ``llama3.2_3b_instruction.json``  per-bucket sizes + total-FLOP counts
  * ``Llama-3.2-3B-Instruct/``        the tokenizer files only (tokenizer.json,
...
# Toggled inside the decode loop to keep the streamed text clean.
import builtins
...
class Llama32_3b_UnifiedEngine(UnifiedEngine):
    ...
    def __init__(self, script_dir: str | None = None):
        # 3B DRAM layout — must match the test script's capture-time layout
        # exactly. PBI bakes absolute JUMP_ABS targets at capture time, so
        # loading the instruction bin at a different program-DRAM address
        # would produce wrong jump targets and garbage execution. The default
        # 768 MB params region is also too small for 3B's ~1.7 GB weights;
        # this layout mirrors Gemma4's full-4-GB layout (memory note
        # ``gemma4_fixed_dram_layout``):
        #   Params  : 0x00000000 – 0x80000000   (2 GiB)
        #   Tensor  : 0x80000000 – 0xA0000000   (512 MiB)
        #   Program : 0xA0000000 – 0x100000000  (1.5 GiB)
        ...
        self.num_q_heads  = self.vector_length // self.actual_head_dim  # 24
        ...
        # reg 4 is left spare. PBI loop counters allocate from reg 5 onward.
        # The test script bumps this counter to 5 at capture time, so all PBI
        # ops in the captured bin assume that reservation. The host-emitted
        # isa_add_set programs that update V_CACHE_SIZE_REG / ROPE_SIZE_REG
        # at runtime use fixed reg indices (set, not allocated), so this
        # counter doesn't affect them — but mirror the test script for parity.
    ...
    def _load_rope_host(self) -> None:
        """Generate the per-head RoPE cos/sin tables on the host and DMA them
        to params DRAM. Tables are deterministic from the rope config — no HF
        lookup needed. For 3B (per-head rope, N=actual_head_dim=128), each
        position row is ``[cos_head(D), cos_head(D), -sin_head(D), sin_head(D)]``
    ...
    def weight_init(self) -> None:
        ...
        tokenizer files. No HF network access — ``local_files_only=True`` on
        the tokenizer fails loudly if tokenizer files are missing."""
        ...
        # Embedding loaded from the weight bin.
    ...
    def tensor_init(self) -> None:
        """Allocate tensor DRAM regions — order and sizes must match the
        capture-time layout in the test script exactly so the loaded
        instruction bin references the right addresses."""
    ...
    def run_decoder(self, decoder_program_sizes: list[int], decoder_base_addr: int, token_id: int,
        ...
        3B-specific strides (per-head rope, not the 1B grouped layout):
          V_CACHE_SIZE_REG = decode_pos × actual_head_dim × bpe  (= 256 B/pos)
          ROPE_SIZE_REG    = decode_pos × 2 × actual_head_dim × bpe  (= 512 B/pos)
        """
        if token_id is None:
        ...
        while self.seq_len < max_seq_len:
            ...
            _kv_stride = self.actual_head_dim * self.bytes_per_element       # 256 B/pos (3B)
            _rope_row  = 2 * self.actual_head_dim * self.bytes_per_element   # 512 B/pos (3B)
...
def _verify_artifacts(script_dir: str) -> None:
    """Hard fail before any FPGA touch if a required file is missing."""
    bin_dir = os.path.join(script_dir, "llama3.2_3b_bin")
    required_files = [
        ("llama3.2_3b_config.json",                                       "model config"),
        ("llama3.2_3b_bin/weights_llama3.2_3b_hf.bin",                    "weight bin"),
        ("llama3.2_3b_bin/llama3.2_3b_instruction.bin",                   "unified instruction bin"),
        ("llama3.2_3b_bin/llama3.2_3b_instruction.json",                  "instruction bin meta"),
    ]
    tokenizer_dir = os.path.join(bin_dir, "Llama-3.2-3B-Instruct")
    tokenizer_required = [
    ...
    if not any(os.path.exists(p) for p, _ in tokenizer_required):
        missing.append(f"  llama3.2_3b_bin/Llama-3.2-3B-Instruct/{{tokenizer.json,tokenizer_config.json}}   (tokenizer files)")
    ...
    if missing:
        ...
        _original_print("  python llama3.2_3b_test.py")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Llama-3.2-3B inference from pre-compiled bins (no HF access).")
    ...
    ue = Llama32_3b_UnifiedEngine(script_dir=script_dir)
