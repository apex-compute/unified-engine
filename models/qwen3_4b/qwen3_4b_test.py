...
Qwen3-4B inference on accelerator: prefill + decode.
  ...
  - Prefill: compiled each run. Decoder: if qwen3_4b_bin/programs.bin and
    qwen3_4b_bin/programs.json exist, skip decoder compile and load
...
Weights:
  - Default: qwen3_4b_bin/params.bin (generated from HF model if missing).
...
def weight_bin_generate(script_dir: str | None = None, output_path: str | None = None) -> str:
    ...
    with open(out_path, "wb") as f:
        f.write(buf)
    meta_rel = paths.get("weights_meta", "qwen3_4b_bin/params.json")
    meta_path = os.path.join(script_dir, meta_rel)
    with open(meta_path, "w") as f:
        json.dump({"size": len(buf)}, f)
...
class Qwen3_4b_UnifiedEngine(UnifiedEngine):
    ...
    def compile_instructions(self, layer_size: int | None = None) -> dict:
        ...
        bin_rel  = paths_cfg.get("instruction_bin",  "qwen3_4b_bin/programs.bin")
        meta_rel = paths_cfg.get("instruction_meta", "qwen3_4b_bin/programs.json")
    ...
    def run_decoder(self, decoder_program_addr: int, preamble_addr: int,
        ...
        the cached decoder program. gpr_seq_len carries across steps via the in-bin
        ADD_INC. Same preamble slot is overwritten per token.
...
def main():
    ...
    if args.local_weights:
        ...
        weights_bin_rel = "qwen3_4b_bin/params.bin"
