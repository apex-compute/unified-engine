...
Usage:
  ...
  python gemma3_test_IF8.py --dev xdma0 [--device kintex7]
...
``--dev`` is the XDMA device name (e.g. ``xdma0``). For Bittware use ``--device bittware``.
...
class Gemma3_UnifiedEngine(UnifiedEngine):
    ...
    def __init__(self, script_dir: str | None = None, local_weights: bool = False, dual_engine: bool = False, engine_slave: bool = False, legacy: bool = False, matmatmul: bool = False, two_pass_prefill: bool = False):
        ...
        # recompute the boundary integers from the (already IF8-rewritten) config
        # sizes. Available physical DRAM is reported separately by HW_INFO.
    ...
    def _compile_decoder_programs_folded(self, layer_size: int = 26, profile: bool = False) -> dict:
        ...
        dot (streams IF8 weights through DOT_PRODUCT, no dequantize) that is fully dynamic in K/N and
        ...
        def fold_matmul(K, N, a_lit, b_lit, out_lit, scale_lit, gpr_K, gpr_N, gelu=False):
            # 1-PASS streaming quantized matvec (quantized_matmat_core_dynamic): streams the IF8
            ...
            return self.quantized_matmat_core(
                ...
                data_type=TYPE.IF8, gelu_enable=gelu,
        ...
        if layer_size == self.LAYER_SIZE:
            ...
            if self.matmatmul:
                ...
                total_flops += self.matmat_mul_core(M=1, K=self.vector_length, N=self.EMBEDDING_ELEMENTS,
                    ...
                    OUTPUT_DRAM_ADDR=self.LOGITS_DRAM, is_B_quantized=True, data_type=TYPE.IF8,
                ...
                total_flops += self.quantized_matmat_core(M=1, K=self.vector_length, N=self.EMBEDDING_ELEMENTS,
                    A_DRAM_ADDR=self.OUTPUT_NORM_DRAM, B_DRAM_ADDR=self.DRAM_ADDR_LM_HEAD_QUANT,
                    OUTPUT_DRAM_ADDR=self.LOGITS_DRAM, data_type=TYPE.IF8,
                    SCALE_DRAM_ADDR=self.DRAM_ADDR_LM_HEAD_SCALE, write_back_disable=True)
    ...
    def _compile_decoder_programs(self, layer_size: int = 26, use_pbi: bool = False, profile: bool = False) -> dict:
        ...
        if use_pbi:
            ...
            # with runtime per-layer addressing and GPR dims — the 1-pass streaming-matmul folded
            # decoder (mirror of the IF4 build). The Python-unrolled body below is the legacy fallback.
