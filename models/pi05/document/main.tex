...
\textbf{Vision head dim.} Head dim $72$ is not $64$-aligned. Attention runs at
$128$ and is contracted back through a precomputed selection matrix; the
$Q/K/V/O$ projections stay at $72$.

\textbf{Prefix masking.} A masked image slot is dropped from prefill, a quarter
less work with bit-identical output. Rotary positions are cumulative-sum-of-mask
indices, not \texttt{arange}, so removing a slot does not shift phase.
...
missing offset scrambles the output silently, with no NaN.
...
takes three, so the third slot is zero-filled and masked. Encoding it costs
nothing in correctness and keeps the three-camera path exercised.
...
%=============================================================================
Table~\ref{tab:platforms} lists the three platforms and their power. Each is
then reported as per-section wall-clock: \alveo{} times come from the engine's
own counters; the GPU baseline is openpi's JAX/XLA path.

\begin{table}[H]
\centering\small
\caption{Platforms. Power is the board draw during inference.}
\label{tab:platforms}
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Platform} & \textbf{Precision} & \textbf{Runtime} & \textbf{Power} \\
\midrule
\alveo{} (AMD Alveo U55C, 12$\times$\ue{}~v1) & \iffour{} weights, \bfteen{} MAC & this work & \numTBD{}\,W \\
RTX~5070                                   & \bfteen{}                        & openpi JAX/XLA & \numTBD{}\,W \\
Jetson Orin Nano Super                     & \bfteen{}                        & openpi PyTorch/CUDA & 7\,W mode \\
\bottomrule
\end{tabular}
\end{table}
...
832-token prefix. Times are pure back-to-back execution; programs are compiled
once up front and that one-time cost is excluded.}
\label{tab:timing-fpga}
\begin{tabular}{@{}lc@{}}
\toprule
\textbf{Model section} & \textbf{Time (s)} \\
\midrule
Vision encoder          & 2.6  \\
Prefix (Gemma-2B)       & 8.6  \\
Action expert (denoise) & 0.9  \\
\midrule
\textbf{End-to-end}     & 12.1 \\
\textbf{Throughput (inf/s)} & 0.0826 \\
\bottomrule
\end{tabular}
\end{table}

The expert is the least efficient stage here for the same reason as on the
Jetson: it is a chain of short sequential steps, not a throughput problem.
...
Jetson. FLOPs concentrate in the prefix; latency concentrates in the expert. And
\iffour{} pays for its footprint only in action dimensions the policy does not
act on.
